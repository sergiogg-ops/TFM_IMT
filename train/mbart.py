...
parser.add_argument('-src_tr','--source_train',default='EuTrans/train/training.en',help='Archivo con el conjunto de entrenamiento en lengua origen.')
parser.add_argument('-src_ev','--source_evaluation',default='EuTrans/dev/development.en',help='Archivo con el conjunto de evaluacion en lengua origen.')
parser.add_argument('-tgt_tr','--target_train',default='EuTrans/train/training.es',help='Archivo con el conjunto de entrenamiento en lengua destino.')
parser.add_argument('-tgt_ev','--target_evaluation',default='EuTrans/dev/development.es',help='Archivo con el conjunto de evaluacion en lengua destino.')
...
if args.max_steps:
    training_args = trans.TrainingArguments(output_dir=args.output_dir,overwrite_output_dir=args.overwrite_output_dir,
...
class Eutrans(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        return {'id':idx,
                'translation':{self.src_lang: tokenize_function(self.source[idx]),
                self.tgt_lang: tokenize_function(self.target[idx])}}

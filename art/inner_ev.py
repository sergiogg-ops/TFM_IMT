...
def read_args():
    ...
    parser.add_argument('-sys','--system', default='sb', choices=['mt','sb','pb'], help='System to compare')
...
def read_file(directory, mode, model, lang):
    wsr = []
    mar = []
    filename = model + '.' + lang if mode == 'mt' else mode + '_imt_' + model + '.' + lang
...
def main():
    ...
    for model1, model2 in tqdm(enumeration, desc='Comprobando significatividad'):
        ...
        try:
            ...
            sig_wsr = sig_wsr<args.pvalue
            sig_mar = sig_mar<args.pvalue
        except FileNotFoundError as error:
            sig_wsr, sig_mar = None, None
            msg = str(error)
            warn(msg, category=RuntimeWarning)
        if args.system == 'mt':
            row['sig_bleu'] = sig_wsr
            row['sig_ter'] = sig_mar
            # significance = significance._append({
            #     'src': args.pair[:2], 
            #     'trg': args.pair[2:], 
            #     'model1':model1, 
            #     'model2':model2,
            ...
            #     'sig_mar': sig_mar
            #     }, ignore_index=True)
        significance = significance._append(row, ignore_index=True)
